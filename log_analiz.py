...
try:
    ...
    print("Eksik modÃ¼l: pip install geoip2 jinja2 requests")
    exit()

# MasaÃ¼stÃ¼ yolu
MASAUSTU = Path.home() / ("MasaÃ¼stÃ¼" if (Path.home() / "MasaÃ¼stÃ¼").exists() else "Desktop")
...
# ISO 8601 formatÄ±ndan tarih Ã§ek
    ...
    for yol in DESTEKLI_LOG_YOLLARI:
        if os.path.exists(yol):
            if yol.endswith(".gz"):
                ...
                return "/tmp/logtemp"
            return yol
    raise FileNotFoundError("Log dosyasÄ± bulunamadÄ±.")
...
def html_rapor_olustur(basarisiz, basarili, brute):
    ...
    with open(MASAUSTU / "guvenlik_raporu.html", "w", encoding="utf-8") as f:
        f.write(html)
    print("âœ… HTML raporu: guvenlik_raporu.html")
...
def json_rapor_olustur(basarisiz, basarili, brute):
    ...
    print("âœ… JSON raporu: guvenlik_raporu.json")
